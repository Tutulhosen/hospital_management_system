...
<div class="page-section">
    <div class="container">
      ...
      <form class="main-form" action="{{route('patient.appoinment.update', $edit_id->id)}}" method="POST">
        @csrf
        <div class="row mt-5 ">
          ...
          <div class="col-12 col-sm-6 py-2 wow fadeInLeft" data-wow-delay="300ms">
            <input value="{{$edit_id->date}}" name="date" type="date" class="form-control">
          </div>
          <div class="col-12 col-sm-6 py-2 wow fadeInRight" data-wow-delay="300ms">
            <select name="doctor" id="doctor" class="custom-select">
              ...
              <option  @if ($edit_id->doctor==$doctor->name)
                selected
              @endif value="{{$doctor->name}}">{{$doctor->name}} --speciality-- {{$doctor->speciality}}</option>
              @empty
